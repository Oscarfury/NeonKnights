async (page) => {
  ...
  await page.screenshot({ path: 'output/playwright/rhythm-company-planning.png' });
  ...
  for (let round = 0; round < 3; round++) {
    ...
    for (let tick = 0; tick < 1400; tick++) {
      ...
      if(threat) heading=Math.PI-threat.wall*Math.PI/2;
      ...
      if (threat && threat.windup-threat.age<.28 && s.guardCooldown===0 && side(s.king)===threat.wall) {
        await page.keyboard.press('KeyE');
        if(!guardCaptured){await page.waitForTimeout(80);await page.screenshot({path:'output/playwright/rhythm-wall-guard.png'});guardCaptured=true;}
      }
      const t = s.dangers.find(t=>t.kind!=='hex');
      if (t) {
        const key = `${t.kind}-${t.age<t.windup?'warning':'impact'}`;
        if (!attacks.has(key)) { attacks.add(key); await page.screenshot({path:`output/playwright/rhythm-${key}.png`}); }
      }
      if (tick === 200) await page.screenshot({path:`output/playwright/rhythm-battle-${round+1}.png`});
      await page.waitForTimeout(95);
    }
    await page.mouse.up();
    const result = await read();
    results.push({ checkpoint, phase: result.phase, time: result.time, guardTime:result.guardTime, stats: result.stats, loot: result.loot, state: result.state });
    await page.evaluate(v=>localStorage.setItem('neon-test:rhythm-results',JSON.stringify(v)),results);
    await page.screenshot({path:`output/playwright/rhythm-result-${round+1}.png`});
    ...
    if (restored.encounter!==round+1) throw Error('Victory did not persist');
    if (restored.knights.some(k=>k.hp!==((k.id==='aldren'||k.id==='corvin'?135:95)+(k.rank-1)*25))) throw Error('Company did not recover');
  }
  if(results[2].stats.perfectGuards<1) throw Error('No timed boss guard recorded');
