...
export function stepDefenses(
  buildings: Defense[],
  targets: { id: number; x: number; z: number; hp: number }[],
  dt: number,
  nextId: () => number,
): SiegeBolt[] {
  ...
  for (const b of buildings) {
    ...
    const candidates = targets.filter((target) => target.hp > 0 && inCoverage(b, target));
    ...
    b.aimYaw += Math.max(-dt * 2.2, Math.min(dt * 2.2, difference));
    if (Math.abs(difference) > 0.06 || b.reload > 0) continue;
    const y = b.rank === 1 ? 1.08 : b.rank === 2 ? 1.43 : 1.78;
    ...
    bolts.push({
      ...
      vy: ((1.4 - y) * 19) / distance,
      vz: Math.cos(b.aimYaw) * 19,
      damage: r.damage,
      life: r.range / 19,
      owner: b.id,
      remaining: b.rank === 3 ? 3 : 1,
      hit: [],
    });
    b.reload = r.interval;
    b.recoil = 0.42;
